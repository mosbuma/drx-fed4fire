...
exec /usr/local/openjdk-8/bin/java -jar /etc/exchange-backend/clients-0.1.jar --server.port=8080 --config.rpc.host=copyright-delta-a --config.rpc.port=10006 --config.rpc.username=user1 --config.rpc.password=test